...
for iwave= ["WoD" "WoR"]
    for itime= ["peak_time" "min_slope_time" "start_time"]
        ...
        %remove nans
        data_med=data_med(~isnan(data_med));
        data_std=data_std(~isnan(data_std));
        data_med_depth=data_med_depth(~isnan(data_med_depth));
        data_mean=data_mean(~isnan(data_mean));
        data_sem=data_sem(~isnan(data_sem));
        data_mean_depth=data_mean_depth(~isnan(data_mean_depth));
        ...
        %remove nans
        data_med_norm=data_med_norm(~isnan(data_med_norm));
        data_std_norm=data_std_norm(~isnan(data_std_norm));
        data_mean_norm=data_mean_norm(~isnan(data_mean_norm));
        data_sem_norm=data_sem_norm(~isnan(data_sem_norm));
...
%         
 ...
 %% Plot WoD & WoR normalized peak time on same graph
        ...
        %remove nans
        data_std_wodnorm=data_std_wodnorm(~isnan(data_std_wodnorm));
        data_mean_wodnorm=data_mean_wodnorm(~isnan(data_mean_wodnorm));
        data_sem_wodnorm=data_sem_wodnorm(~isnan(data_sem_wodnorm));
        data_std_wornorm=data_std_wornorm(~isnan(data_std_wornorm));
        data_mean_wornorm=data_mean_wornorm(~isnan(data_mean_wornorm));
        data_sem_wornorm=data_sem_wornorm(~isnan(data_sem_wornorm));
...
for iwave=["WoD" "WoR"]
    for ivalue= ["peak_value" "min_slope_value" "half_width"]

        if ivalue=="half_width" && iwave=="WoR"
            continue
        end
        
        
         %interpolate missing values
        for irat = 1:size(ordered_data.(iwave).(ivalue), 1)
            startinterp = find(~isnan(ordered_data.(iwave).(ivalue)(irat,:)), 1, 'first');
            endinterp = find(~isnan(ordered_data.(iwave).(ivalue)(irat,:)), 1, 'last');
            ordered_data_interp.(iwave).(ivalue)(irat,startinterp:endinterp) = fillmissing(ordered_data.(iwave).(ivalue)(irat,startinterp:endinterp), 'linear'); %ou pchip, ou spline
            ordered_data_interp.Depth(irat,startinterp:endinterp)= fillmissing(ordered_data.Depth(irat,startinterp:endinterp), 'linear');
        end
        
        %replace zeros by NaNs
        idx_zeros= find(ordered_data_interp.(iwave).(ivalue)(:,:)==0);
        idx_zeros_depth=find(ordered_data_interp.Depth(:,:)==0);
        ordered_data_interp.(iwave).(ivalue)(idx_zeros)=nan;
        ordered_data_interp.Depth(idx_zeros_depth)=nan;
        
        %calculate median and standard deviation
        for ichan= 1:22%size(ordered_data.(iwave).(ivalue),2)
            data_med_value(1,ichan)= nanmedian(ordered_data_interp.(iwave).(ivalue)(:,ichan));
            data_std_value(1,ichan)= std(ordered_data_interp.(iwave).(ivalue)(:,ichan),'omitnan');
            data_med_depth(1,ichan)=nanmedian(ordered_data_interp.Depth(:,ichan));
            data_mean_value(1,ichan)=nanmean(ordered_data_interp.(iwave).(ivalue)(:,ichan));
            data_sem_value=data_std / (sqrt(length(ordered_data_interp.(iwave).(ivalue))));
            data_mean_depth(1,ichan)=nanmean(ordered_data_interp.Depth(:,ichan));
        end %ichan
        
        
         %remove nans
        data_med_value=data_med_value(~isnan(data_med_value));
        data_std_value=data_std_value(~isnan(data_std_value));
        data_med_depth=data_med_depth(~isnan(data_med_depth));
        data_mean_value=data_mean_value(~isnan(data_mean_value));
        data_sem_value=data_sem_value(~isnan(data_sem_value));
        data_mean_depth=data_mean_depth(~isnan(data_mean_depth));
        
        %% Plot mean +/- Sem normalized
        
        fig_meannorm=figure;hold
        sgtitle(sprintf('%s %s time normalized mean',iwave,ivalue));
