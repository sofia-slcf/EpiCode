...
for iwave= ["WoD" "WoR"]
    ...
    idxdepth_names={'superficial','deep','outlier'};


        %interpolate missing values
        for irat = 1:size(ordered_data.(iwave).peak_time, 1)
            startinterp = find(~isnan(ordered_data.(iwave).peak_time(irat,:)), 1, 'first');
            endinterp = find(~isnan(ordered_data.(iwave).peak_time(irat,:)), 1, 'last');
            ordered_data_interp.(iwave).peak_time(irat,startinterp:endinterp) = fillmissing(ordered_data.(iwave).peak_time(irat,startinterp:endinterp), 'linear'); %ou pchip, ou spline
        ...
        idx_zeros= find(ordered_data_interp.(iwave).peak_time(:,:)==0);
        idx_zeros_depth=find(ordered_data_interp.Depth(:,:)==0);
        ordered_data_interp.(iwave).peak_time(idx_zeros)=nan;
        ordered_data_interp.Depth(idx_zeros_depth)=nan;
        
        %calculate median and standard deviation
        for ichan= 1:22%size(ordered_data.(iwave).peak_time,2)
            data_med(1,ichan)= nanmedian(ordered_data_interp.(iwave).peak_time(:,ichan));
            data_std(1,ichan)= std(ordered_data_interp.(iwave).peak_time(:,ichan),'omitnan');
            data_med_depth(1,ichan)=nanmedian(ordered_data_interp.Depth(:,ichan));
            data_mean(1,ichan)=nanmean(ordered_data_interp.(iwave).peak_time(:,ichan));
            data_sem=data_std / (sqrt(length(ordered_data_interp.(iwave).peak_time)));
        ...
        %% plot raw timings
        fig_raw=figure;
        sgtitle(sprintf('%s %s time',iwave,peak_time));
        ...
        for irat= 1:size(ordered_data.(iwave).peak_time, 1)
            ...
            %plot each protocol
            plot(ordered_data_interp.(iwave).peak_time(irat,:),ordered_data_interp.Depth(irat,:),'Color',C,'LineWidth',1);
            hold on;
        ...
        fname_raw=fullfile(cfg{4}.imagesavedir,'delays',sprintf('%s_%s_raw',iwave,peak_time));
        ...
        sgtitle(sprintf('%s %s time raw median',iwave,"peak_time"));
        ...
        fname_medraw=fullfile(cfg{4}.imagesavedir,'delays',sprintf('%s_%s_median_raw',iwave,"peak_time"));
        ...
        sgtitle(sprintf('%s %s time raw mean',iwave,peak_time));
        ...
        fname_meanraw=fullfile(cfg{4}.imagesavedir,'delays',sprintf('%s_%s_mean_raw',iwave,"peak_time"));
        ...
        for irat=1:size(ordered_data.(iwave).peak_time, 1)
            data_plot_norm(irat,:)=ordered_data_interp.(iwave).peak_time(irat,:) -min(ordered_data_interp.(iwave).peak_time(irat,:));
        end %irat
        
        %calculate median and standard deviation
        for ichan= 1:22%size(ordered_data.(iwave).peak_time,2)
            data_med_norm(1,ichan)= nanmedian(data_plot_norm(:,ichan));
            data_std_norm(1,ichan)= std(ordered_data_interp.(iwave).peak_time(:,ichan),'omitnan');
            data_mean_norm(1,ichan)=nanmean(data_plot_norm(:,ichan));
            data_sem_norm=data_std_norm / (sqrt(length(data_plot_norm)));
        end %ichan
        
        fig_norm=figure;
                sgtitle(sprintf('%s %s time normalized',iwave,peak_time));
        ...
        fname_norm=fullfile(cfg{4}.imagesavedir,'delays',sprintf('%s_%s_norm',iwave,"peak_time"));
        ...
        sgtitle(sprintf('%s %s time normalized median',iwave,peak_time));
        ...
        fname_mednorm=fullfile(cfg{4}.imagesavedir,'delays',sprintf('%s_%s_median_norm',iwave,"peak_time"));
        ...
        sgtitle(sprintf('%s %s time normalized mean',iwave,"peak_time"));
        ...
        fname_meannorm=fullfile(cfg{4}.imagesavedir,'delays',sprintf('%s_%s_mean_norm',iwave,"peak_time"));
 ...
 %% Plot WoD & WoR normalized peak time on same graph
         ...
         for irat=1:size(ordered_data.WoD.peak_time, 1)
