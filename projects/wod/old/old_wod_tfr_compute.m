...
%03/10/2021 sofia modified : line 76  & line 122
if isempty(cfg)
    return
end
...
for itrial = 1:size(LFP.trial,2)
    ...
    for ichan = 1:size(timefreq_alldata{itrial}.short.label,1)%(timefreq_alldata{itrial}.long.label,1)
        ...
        %select channel for short param%long param
        ichan_name              = timefreq_alldata{itrial}.short.label{ichan};%timefreq_alldata{itrial}.long.label{ichan};
        cfgtemp                 = [];
        cfgtemp.channel         = ichan_name;
        timefreq_ichan_temp   	= ft_selectdata(cfgtemp,timefreq_alldata{itrial}.short);%ft_selectdata(cfgtemp,timefreq_alldata{itrial}.long);
