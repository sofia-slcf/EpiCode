...
elseif isunix
    path_tab        = '/network/lustre/iss01/charpier/raw/rat-wod/2probes/Data_infos';
    os          = 'unix';
elseif ispc
    path_tab        = '\\lexport\iss01.charpier\raw\rat-wod\2probes\Data_infos';
    os          = 'windows';
else
    error('Platform not supported')
end
...
Table_experience = readtable(fullfile(path_tab,'Tableau_Experience_per_rat.xlsx'))
...
protocol = dir(fullfile(path_tab,'grid'));
...
for irat=1:size(Table_experience,1) %pour chaque les rats
    ...
    Total_nb_electrode = Table_experience(irat,9)+Table_experience(irat,16);
    ...
    tableau(irat).eachshaped= NaN(Total_nb_electrode,23)
    nbNaN=[]
    for row=1:size(tableau(irat).eachshaped,1)
        ligne= row-length(nbNaN) %ligne de la grid
        array= Total_nb_electrode+1-row %ligne du tableau
        if ligne <= size(tableau(irat).each,1)
    ...
    savedir= fullfile(path_tab,'maxi_cut')
      if ~exist(savedir)
         mkdir(savedir);
     end
    ...
    clearvars -except  protocol folder Table_experience tableau.each path_tab
