    ...
    for ipatient = 1 : 7
        ...
        for ipart = 1 : size(SpikeTrials_timelocked, 2)
            ...
            for ilabel = 1 : size(SpikeStats_windowed{ipart}.window, 2)
                ...
                % add to windowed
                temp.purity                 = repmat(SpikeTrials_timelocked{ipart}.combined1.purity(ilabel), size(temp, 1), 1);
                temp.cluster_group          = repmat(string(SpikeTrials_timelocked{ipart}.combined1.cluster_group{ilabel}), size(temp, 1), 1);
